// ============================================================
// Обработчик заявления об отмене судебного приказа. Раньше общая
// логика жила в отдельном файле simple-flow-handler-factory.js, но
// этот файл упорно не доходил до сервера при загрузке — поэтому код
// теперь целиком здесь.
// ============================================================
...
const FLOW_PATH = path.join(__dirname, "..", "flows", "cancel-court-order.json");
const OUTPUT_FILENAME = "Заявление об отмене судебного приказа.docx";
const CAPTION = "📄 Возражения относительно исполнения судебного приказа готовы. Важно успеть подать их в течение 10 дней с даты получения приказа (ст. 128 ГПК РФ) — приложите этот файл и копию паспорта.";
const RESUME_COMMAND = "cancelorder";
...
async function renderCurrentStep(chatId, engine, deps) {
  try {
    if (engine.isFinished()) {
      const buffer = await generateCancelCourtOrderApplication(engine.collectedData);
      await deps.sendDocument(chatId, buffer, OUTPUT_FILENAME, CAPTION);
      return;
    }
    ...
    console.error("Ошибка на шаге (cancel-court-order)", engine.currentNodeId, ":", e);
    await deps.sendMessage(chatId, `⚠️ Что-то пошло не так — попробуйте написать /${RESUME_COMMAND} ещё раз.`);
  }
}
...
async function handleAction(chatId, engine, action, deps) {
  ...
  try {
    ...
    console.error("Ошибка при обработке шага (cancel-court-order)", engine.currentNodeId, ":", e);

...
async function renderCurrentStep(chatId, engine, deps) {
  ...
  if (node.type === "upload") {
    await deps.sendMessage(chatId, `📎 ${node.title}`, skipKeyboard(node));
    return;
  }
  ...
  if (node.type === "collection") {
    ...
    if (awaiting === "item") {
      await deps.sendMessage(chatId, `📎 ${node.itemPrompt}`, collectionItemKeyboard());
    } else {
...
// action: { type: 'photo'|'text'|'skip'|'callback', payload }

// ============================================================
// Обработчик заявления приставам о сохранении прожиточного минимума.
// Раньше общая логика жила в отдельном файле
// simple-flow-handler-factory.js, но этот файл упорно не доходил до
// сервера при загрузке — поэтому код теперь целиком здесь.
// ============================================================
...
const FLOW_PATH = path.join(__dirname, "..", "flows", "pristav-minimum.json");
const OUTPUT_FILENAME = "Заявление приставам о сохранении прожиточного минимума.docx";
const CAPTION = "📄 Заявление о сохранении прожиточного минимума готово. Подать можно лично приставу, по почте или через Госуслуги.";
const RESUME_COMMAND = "minimum";
...
async function renderCurrentStep(chatId, engine, deps) {
  try {
    if (engine.isFinished()) {
      const buffer = await generatePristavMinimumApplication(engine.collectedData);
    ...
    console.error("Ошибка на шаге (pristav-minimum)", engine.currentNodeId, ":", e);
...
async function handleAction(chatId, engine, action, deps) {
  ...
  try {
    ...
    console.error("Ошибка при обработке шага (pristav-minimum)", engine.currentNodeId, ":", e);

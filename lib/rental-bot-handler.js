// ============================================================
// Обработчик договора аренды квартиры. Раньше общая логика жила в
// отдельном файле simple-flow-handler-factory.js, но этот файл
// упорно не доходил до сервера при загрузке (несколько попыток подряд
// давали "модуль не найден", хотя на GitHub всё выглядело правильно) —
// поэтому код теперь просто целиком здесь, без внешней зависимости.
// ============================================================
...
const OUTPUT_FILENAME = "Договор аренды квартиры.docx";
const CAPTION = "📄 Черновик договора аренды готов. Обязательно проверьте все данные перед подписанием.";
const RESUME_COMMAND = "rent";
...
async function renderCurrentStep(chatId, engine, deps) {
  try {
    ...
    console.error("Ошибка на шаге (rental-agreement)", engine.currentNodeId, ":", e);
...
async function handleAction(chatId, engine, action, deps) {
  ...
  try {
    ...
    console.error("Ошибка при обработке шага (rental-agreement)", engine.currentNodeId, ":", e);

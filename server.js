...
const {
  ...
  answerCallbackQuery, editMessageReplyMarkup, deleteMyCommands, setChatMenuButton,
...
server.listen(PORT, async () => {
  ...
  // Убираем значок ☰ (меню команд) — все действия теперь доступны только
  // через видимые inline-кнопки под сообщениями бота.
  try {
    const cmdResult = await deleteMyCommands();
    console.log("deleteMyCommands result:", cmdResult);
    const menuResult = await setChatMenuButton({ type: "default" });
    console.log("setChatMenuButton result:", menuResult);
  } catch (e) {
    console.error("Не удалось убрать меню команд:", e.message);
  }
});

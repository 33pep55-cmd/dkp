...
const {
  ...
  AlignmentType, Table, TableRow, TableCell, WidthType, BorderStyle, TableLayoutType,
} = require("docx");
...
function fullName(data) {
  return data.fio || [data.surname, data.firstName, data.patronymic].filter(Boolean).join(" ");
}

// OCR отдаёт паспорт одной строкой "45 12 678910" — тексту заявления
// нужны серия и номер раздельно.
...
// Раскрытие сделок за 3 года — обязательное по ст. 213.4 ФЗ №127-ФЗ,
// раньше сделки упоминались только в списке приложений, а в самом
// тексте заявления не описывались вовсе.
function dealsText(data) {
  const deals = data.deals || [];
  if (deals.length === 0) {
    return "Сделки с имуществом на сумму свыше 300 000 рублей за три года, предшествующие подаче настоящего заявления, Должником не совершались.";
  }
  const items = deals.map(d => {
    if (d.enteredManually && d.raw) return d.raw;
    const parts = [
      d.dealType || "сделка",
      d.propertyType ? `(${d.propertyType})` : "",
      d.propertyAddress || d.carModel || "",
      d.dealAmount ? `на сумму ${formatMoney(d.dealAmount)} руб.` : "",
      d.dealDate ? `от ${d.dealDate}` : "",
      d.counterpartyFio ? `со стороной ${d.counterpartyFio}` : "",
    ].filter(Boolean);
    return parts.join(" ");
  }).join("; ");
  return `За три года, предшествующие подаче настоящего заявления, Должником совершены следующие сделки с имуществом на сумму свыше 300 000 рублей: ${items}.`;
}
...
async function generateBankruptcyApplication(data) {
  ...
  // OCR (claude.js) отдаёт часть полей под другими именами (fio/dob/pob/
  // passport одной строкой/address), чем ожидает сам текст заявления —
  // нормализуем один раз здесь, а не гадаем по всему файлу.
  const passportParts = splitPassport(data.passport);
  const birthDate = data.birthDate || data.dob || "—";
  const birthPlace = data.birthPlace || data.pob || "—";
  const passportSeries = data.passportSeries || passportParts.series || "—";
  const passportNumber = data.passportNumber || passportParts.number || "—";
  const passportIssuedDate = data.passportIssuedDate || data.issuedDate || "—";
  const passportIssuedBy = data.passportIssuedBy || data.issuedBy || "—";
  const passportDeptCode = data.passportDeptCode || data.deptCode || "—";
  const regAddress = data.regAddress || data.address || "—";
  ...
  // Ширины колонок в DXA (1/20 пункта) — короче для коротких колонок
  // ("№ п/п", "Санкции"), шире для тех, где длинные названия и полные
  // юридические адреса. Раньше все восемь колонок были одной и той же
  // ширины, из-за чего длинный текст переносился по одной букве.
  const COL_WIDTHS = [500, 1100, 1700, 2200, 1400, 900, 900, 938];

  const creditorRows = creditors.map((c, i) => new TableRow({
    children: [
      cell(`1.${i + 1}`, false, COL_WIDTHS[0]), cell("Кредитор", false, COL_WIDTHS[1]), cell(c.creditorName || "—", false, COL_WIDTHS[2]),
      cell(c.creditorAddress || "не установлено", false, COL_WIDTHS[3]),
      cell(c.creditorBasis || (c.creditorKind === "МФО" ? "Согласно кредитной истории" : "Согласно кредитной истории «Кредистория»"), false, COL_WIDTHS[4]),
      cell(formatMoney(c.totalDebt), false, COL_WIDTHS[5]), cell(formatMoney(c.overdueDebt || c.totalDebt), false, COL_WIDTHS[6]), cell("-", false, COL_WIDTHS[7]),
    ],
  }));
  ...
  const doc = new Document({
    sections: [{
      ...
      children: [
        ...
        P([R(`Дата рождения: ${birthDate}`)], { spacing: { after: 50 } }),
        P([R(`Место рождения: ${birthPlace}`)], { spacing: { after: 50 } }),
        P([R(`СНИЛС ${data.snils || "—"}`)], { spacing: { after: 50 } }),
        P([R(`ИНН ${data.inn || "—"}`)], { spacing: { after: 50 } }),
        P([R(`Паспорт серия: ${passportSeries} № ${passportNumber}`)], { spacing: { after: 50 } }),
        P([R(`выдан ${passportIssuedDate}, ${passportIssuedBy}, код подразделения ${passportDeptCode}`)], { spacing: { after: 50 } }),
        P([R(`Адрес регистрации: ${regAddress}`)], { spacing: { after: 300 } }),

        P([R("Кредиторы:", { bold: true })], { spacing: { after: 100 } }),
        // В шапке каждый кредитор — один раз, даже если у него несколько
        // кредитов (например, две карты в одном банке). В таблице ниже
        // все записи по-прежнему перечисляются по отдельности.
        ...Array.from(new Set(creditors.map(c => c.creditorName || "—")))
          .map((name, i) => P([R(`${i + 1}. ${name}`)], { spacing: { after: 30 } })),
        ...
        P([R(`${name} (дата рождения: ${birthDate} г.р., место рождения: ${birthPlace}, СНИЛС ${data.snils || "—"}, ИНН ${data.inn || "—"}, паспорт: ${passportSeries} ${passportNumber}, выдан ${passportIssuedDate}, ${passportIssuedBy}, код подразделения ${passportDeptCode}, адрес регистрации: ${regAddress}, обращается в арбитражный суд с настоящим заявлением в порядке ст. 213.4 Федерального закона от 26.10.2002 г. № 127-ФЗ «О несостоятельности (банкротстве)», что обусловлено его(её) неплатёжеспособностью и неспособностью погасить кредиторскую задолженность в полном объёме.`)], { spacing: { after: 300 } }),
        ...
        new Table({
          ...
          rows: [
            new TableRow({ children: [
              cell("№ п/п", true, COL_WIDTHS[0]), cell("Содержание обязательства", true, COL_WIDTHS[1]), cell("Кредитор", true, COL_WIDTHS[2]),
              cell("Место нахождения кредитора", true, COL_WIDTHS[3]), cell("Основание возникновения", true, COL_WIDTHS[4]),
              cell("Всего", true, COL_WIDTHS[5]), cell("В т.ч. задолженность", true, COL_WIDTHS[6]), cell("Санкции", true, COL_WIDTHS[7]),
            ]}),
            ...creditorRows,
          ],
        }),
        ...
        P([R(dealsText(data))], { spacing: { after: 200 } }),

...
async function renderCurrentStep(chatId, engine, deps) {
  try {
    ...
    if (node.type === "creditors_review") {
      ...
      await deps.sendMessage(chatId, "🔎 Проверьте кредиторов, найденных в обоих отчётах — один и тот же кредит мог попасть в оба отчёта сразу. Снимите галочку с задвоившихся или лишних пунктов, затем нажмите «Готово».", withBack(buildCreditorsReviewKeyboard(creditors)));
      return;
    }
...
// Вынесено отдельно, чтобы можно было перестроить те же самые кнопки
// после переключения галочки — без этого пришлось бы дублировать
// логику построения строк в server.js.
function buildCreditorsReviewKeyboard(creditors) {
...
module.exports = { newEngine, renderCurrentStep, handleAction, STUB_TEMPLATES, buildCreditorsReviewKeyboard, withBack };

...
// Скачивает файл-документ (например, PDF отчёта "Кредистории") и
// возвращает его как Buffer — в отличие от фото, тут не нужен base64,
// файл просто сохраняется на диск для дальнейшего разбора.
async function downloadDocument(document) {
  ...
  const fileRes = await fetch(`${FILE_API}/${filePath}`);
  const arrBuf = await fileRes.arrayBuffer();
  return { buffer: Buffer.from(arrBuf), fileName: document.file_name || "document.pdf" };
}

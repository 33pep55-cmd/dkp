...
function newSession(mode = "full") {
  return {
    mode, // 'full' — весь ДКП, 'gibdd_only' — только заявление в ГИБДД
    stepIndex: 0,
...
// Короткий сценарий "только заявление в ГИБДД" — для тех, кому не нужен
// сам ДКП, а нужно только заявление на постановку на учёт (например,
// договор уже есть готовый с прошлого раза).
const STEPS_GIBDD = [
  { key: "buyer_main", docType: "passport_main", target: "buyer", prompt: "📄 <b>Шаг 1 из 3</b>\nПришлите фото разворота паспорта <b>ПОКУПАТЕЛЯ</b> с фотографией." },
  { key: "buyer_reg", docType: "passport_registration", target: "buyer", prompt: "📍 <b>Шаг 2 из 3</b>\nТеперь пришлите разворот паспорта <b>ПОКУПАТЕЛЯ</b> со штампом регистрации." },
  { key: "vehicle", docType: "vehicle_doc", target: "vehicle", prompt: "🚗 <b>Шаг 3 из 3</b>\nИ последнее фото — СТС или ПТС на автомобиль." },
];
const GIBDD_ONLY_INTRO =
  "🚦 <b>Заявление в ГИБДД</b>\n\n" +
  "Соберу только заявление на постановку автомобиля на учёт — без договора купли-продажи. Пришлите документы покупателя и авто.\n\n";
...
// Двухуровневое меню: сначала категория документа, потом конкретный
// вариант внутри неё. Так проще добавлять новые виды документов
// в будущем, не захламляя самый первый экран.
const CATEGORY_KEYBOARD = {
  inline_keyboard: [
    [{ text: "🚗 Договор купли-продажи авто", callback_data: "cat_dkp" }],
    [{ text: "📋 Заявление о банкротстве", callback_data: "cat_bankrot" }],
  ],
};
const DKP_DOCS_KEYBOARD = {
  inline_keyboard: [
    [{ text: "📑 Полный комплект (ДКП + ГИБДД)", callback_data: "menu_new" }],
    [{ text: "🚦 Только заявление в ГИБДД", callback_data: "menu_gibdd_only" }],
    [{ text: "◀️ Назад", callback_data: "menu_categories" }],
  ],
};
const BANKROT_DOCS_KEYBOARD = {
  inline_keyboard: [
    [{ text: "📋 Полное заявление о банкротстве", callback_data: "menu_bankruptcy" }],
    [{ text: "◀️ Назад", callback_data: "menu_categories" }],
  ],
};
...
async function handlePhoto(chatId, session, photoArray) {
  const stepsArray = session.mode === "gibdd_only" ? STEPS_GIBDD : STEPS;
  const step = stepsArray[session.stepIndex];
  if (!step) {
  ...
  try {
    ...
    session.stepIndex += 1;
    const next = stepsArray[session.stepIndex];
    if (next) {
      await sendStepGuide(chatId, next, "✅ Готово.\n\n");
    } else if (session.mode === "gibdd_only") {
      // В коротком сценарии продавца нет вообще — сразу к телефону покупателя.
...
async function handleBuyerPhone(chatId, session, text) {
  ...
  if (session.mode === "gibdd_only") {
    // Договор не нужен вообще — сразу к данным для самого заявления в ГИБДД.
    session.awaitingInn = true;
    await sendGuideAnimation(chatId, STEP_ASSETS.inn, `✅ Принято.\n\n${INN_PROMPT}`, INN_INLINE_KEYBOARD);
    return;
  }
...
async function handleCallbackQuery(callbackQuery) {
  ...
  if (data === "cat_dkp") {
    await sendMessage(chatId, "🚗 Выберите, что нужно сформировать:", DKP_DOCS_KEYBOARD);
    return;
  }
  if (data === "cat_bankrot") {
    await sendMessage(chatId, "📋 Выберите документ:", BANKROT_DOCS_KEYBOARD);
    return;
  }

  if (data === "menu_new" || data === "menu_reset") {
    sessions.set(chatId, newSession("full"));
    await sendStepGuide(chatId, STEPS[0], INTRO);
    return;
  }
  if (data === "menu_gibdd_only") {
    sessions.set(chatId, newSession("gibdd_only"));
    await sendStepGuide(chatId, STEPS_GIBDD[0], GIBDD_ONLY_INTRO);
    return;
  }
...
async function handleUpdate(body) {
  ...
  if (text === "/start") {
    ...
    await sendMessage(chatId, "👋 Какой документ хотите сформировать?", START_KEYBOARD);
    return;
  }
  if (text === "/new") {
    sessions.set(chatId, newSession("full"));
